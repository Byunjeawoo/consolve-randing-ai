...
export default function TechStack() {
  return (
    <section id="tech" className="py-24 bg-slate-900 overflow-hidden" aria-labelledby="tech-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-700/60 text-slate-300 text-sm font-medium mb-4 border border-slate-700">
            ...
            <div key={cat.category} className="flex flex-col sm:flex-row sm:items-start gap-4">
              <div className="sm:w-40 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-300">{cat.category}</span>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {cat.techs.map((tech) => (
                  <span
                    key={tech}
                    className="font-mono text-sm px-3 py-1.5 rounded-lg bg-slate-800/80 text-slate-200 border border-slate-700/60 hover:border-slate-500 transition-colors duration-150"
                  >
                    {tech}
                  </span>
                ))}
